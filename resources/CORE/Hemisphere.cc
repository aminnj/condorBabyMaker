    ...
    // for the other objects
      ...
      // or the Lund distance (method 3)
              if (hemi_meth == 3) {
                ...
                //mass2 *= NewAxis2_E/((NewAxis2_E+Object_E[i])*(NewAxis1_E+Object_E[i])); //commented out by jgran
                mass2 *= NewAxis2_E/((NewAxis2_E+Object_E[i])*(NewAxis2_E+Object_E[i])); //fix by jgran
              }
      // and associate the object to the best hemisphere and add it to the sum
              //cout << "NewAxis1: " << NewAxis1_E << endl;
              //cout << "NewAxis2: " << NewAxis2_E << endl;
              //cout << "lund dists: " << mass1 << ", " << mass2 << endl;
              if (mass1 < mass2) {
                if (Object_Group[i] != 1){
                  I_Move = true;
                }
